...
class ABIService {
  ...
  async loadAllAbis() {
    try {
      const storedAbis = await dbService.getAllVerifiedContracts()
      storedAbis.forEach(item => {
        this.abiCache.set(item.address.toLowerCase(), item.abi)
        if (item.name) {
          this.nameCache.set(item.address.toLowerCase(), item.name)
        }
      })
    } catch (error) {
  ...
  async getAbi(address: string): Promise<Abi | null> {
    ...
    // Load from DB
    try {
      const stored = await dbService.getAbiData(normalizedAddress)
      if (stored) {
        this.abiCache.set(normalizedAddress, stored.abi)
        if (stored.name) {
          this.nameCache.set(normalizedAddress, stored.name)
        }
        return stored.abi
      }
    } catch (error) {
  ...
  getMethodName(input: string): string {
    return this.getMethodSignature(input)
  }
}
